...
class ResNet_Bottleneck_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 50:
            ...
            # a=torch.load("./ptsemseg/models/imagenet/50_imagenet.pth.tar")
            # load pretrained model:
            resnet.load_state_dict(torch.load("./ptsemseg/models/imagenet/50_imagenet.pth.tar"))
            ...
            # pp=resnet.state_dict()
            all=list(resnet.children())
            c=all[:4]
            c1=list(resnet.children())[4]
            c2=c1[0]
            # ppp=c2.state_dict()
            c3=c1[1]
            self.c5 = nn.Sequential(*c, c2, c3)

            d=list(resnet.children())[5:-3]
            d1=c1[2]
            # d2=[d1,d]
            self.d2=nn.Sequential(d1,*d)
            d3=c1
            # self.resnet=nn.Sequential(c4,d2)

            # b=nn.Sequential(*list(resnet.children())[:-3])
    ...
    def forward(self, x):
        ...
        low_x=self.c5(x)
        output=self.d2(low_x)
        output = self.layer5(output) # (shape: (batch_size, 4*512, h/16, w/16))
